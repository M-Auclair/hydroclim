#' hydro_plot_dayofyear_cumulative
#'
...
hydro_plot_dayofyear_cumulative <- function(
    station_number,
    parameter,
    select_year = 2024,
    after_bennett = FALSE,
...

{
  ...
  if(interactive == F){
    y_axis_title <- hydro_parameter(parameter = parameter)[[3]] #this axis title doesn't work with plotly
  }

  # Check for line_colours
  if(length(select_year) > length(line_colours)) {
    line_colours = rainbow(length(select_year))
  } else if(length(select_year) < length(line_colours)) {
    line_colours = line_colours[1:length(select_year)]
  }

  # Import data
  daily_stats <- hydro_calc_dayofyear_cumulative(
    parameter = parameter,
  ...
  # Plot the graph
  plot <- ggplot2::ggplot(daily_stats, ggplot2::aes(x = DayofYear, y = cum_value)) +
